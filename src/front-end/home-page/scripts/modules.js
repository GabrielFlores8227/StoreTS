...
export function buildHead(header) {
...
export function buildAsideMenus(products) {
...
export function buildSavedAsideMenu(products) {
	...
	Object.keys(products).forEach((categoryKey) => {
		products[categoryKey].forEach((product) => {
			...
			template.style.height = template.offsetHeight + 28 + 'px';
		});
	});
}

export function buildCategoriesAsideMenu(products) {
...
export function buildPropagandas(propagandas, footer) {
...
export function buildProducts(products) {
...
export function buildSearchBar(products) {
					...
					productsForKey.forEach((product) => {
						...
						template.addEventListener('click', () => {
							setTimeout(() => {
								...
								scrollTop(productSliderContainer);
								setTimeout(() => {
									scrollLeft(productSliderContainer, document.querySelector('[product-' + product.id + ']'));
								}, 1200);
							}, 120);
						});
...
export function updateLoadingProgessBar(progress) {
...
export function handleNotifications(header, products) {
	if ('Notification' in window) {

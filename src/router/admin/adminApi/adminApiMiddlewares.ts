...
class Sql {
	...
	private static readonly sqlSocketPath: string =
		process.env['SQL_SOCKET_PATH']!;

	private static readonly sqlMasterUser: string =
		process.env['SQL_MASTER_USER']!;
	private static readonly sqlMasterPassword: string =
		process.env['SQL_MASTER_PASSWORD']!;
	...
	public static async sqlMasterQuery(
		command: string,
		values: string[] = [],
	): Promise<
		[
...
class S3 {
	...
	private static readonly s3BucketRegion: string =
		process.env['S3_BUCKET_REGION']!;
	private static readonly s3AccessId: string = process.env['S3_ACCESS_ID']!;
	private static readonly s3SecretAccessKey: string =
...
class MiddlewareUtil {
	...
	private static async sharpImage(
		imageBuffer: Buffer,
		width: number,
		height: number,
	...
	public static async deleteProduct(product: {
		id: number;
		image: string;
	}): Promise<void> {
		await S3.deleteFileFromS3Bucket(product.image);

		await Sql.sqlMasterQuery('DELETE FROM products WHERE id = ?;', [
			String(product.id),
		]);
	}

	public static async validateDataForMiddlewarePostPropaganda(
		body: { imagesContext: [string, string] },
		files:
		...
		if (!files || files.length !== 2) {
			throw {
				type: 400,
				message:
					'Por favor, forneça todas as imagens corretamente para concluir a solicitação',
			};
		}
		...
		try {
			...
			if (err instanceof TypeError) {
				throw {
					type: 400,
					message:
						'Por favor, forneça todos os dados corretamente para concluir a solicitação',
				};
			}
			...
			if (err.message === 'Input buffer contains unsupported image format') {
				throw {
					type: 400,
					message:
						'Por favor, forneça imagens válidas para concluir a solicitação',
				};
			}
	...
	public static async validateDataForMiddlewarePostCategory(body: {
		category: string;
	}): Promise<void> {
		try {
			var [query]: [
		...
		if (Object(query).length !== 0) {
			throw {
				type: 400,
				message:
					'Por favor, forneça um nome de categoria diferente para concluir a solicitação',
			};
		}
	}
	...
	public static async validateDataForMiddlewarePostProduct(
		...
		if (!files || files.length !== 1) {
			throw {
				type: 400,
				message:
					'Por favor, forneça uma imagem válida para concluir a solicitação',
			};
		}
		...
		if (Object(query).length !== 1) {
			throw {
				type: 400,
				message:
					'Por favor, forneça um nome de categoria existente para concluir a solicitação',
			};
		}
		...
		try {
			...
			productImage.buffer = await this.sharpImage(
				productImage.buffer,
				800,
				800,
				'contain',
	...
	public static async validateDataForMiddlewarePutLogin(body: {
		oldUser: string;
		oldPassword: string;
		newUser: string;
		newPassword: string;
		...
		const [query]: [
			...
			'SELECT user, password FROM auth WHERE user = ? AND password = ?;',
			[body.oldUser, body.oldPassword],
		);
	...
	public static validateDataForMiddlewarePutText(body: {
		id: string;
		table: string;
		column: string;
		value: string;
		...
		if (
			!Object(sqlMask)[body.table] ||
			!Object(sqlMask)[body.table].includes(body.column)
		) {
			throw {
				type: 400,
				message:
	...
	public static async validateDataForMiddlewarePutImage(
		body: { id: string; table: string; column: string },
		files:
			| { [fieldname: string]: Express.Multer.File[] }
			| Express.Multer.File[]
			| undefined,
		...
		if (
			!Object(sqlMask)[body.table] ||
			!Object(sqlMask)[body.table][body.column]
		) {
			throw {
				type: 400,
				message:
		...
		try {
			...
			image.buffer = await this.sharpImage(
				image.buffer,
				Object(sqlMask)[body.table][body.column].width,
				Object(sqlMask)[body.table][body.column].height,
				'contain',
			);
		} catch (err: any) {
			if (err instanceof TypeError) {
	...
	public static async validateDataForMiddlewarePutPosition(body: {
		table: string;
		ids: string[];
	}) {
		...
		const [query]: [
			...
			'SELECT id FROM ' + body.table + ' ORDER BY position;',
		);
	...
	public static handleMiddlewareError(res: express.Response, err: any): void {
		if (err.type && typeof err.type === 'number') {
			...
			res.status(500).json({
				status: 500,
				message:
					'Por favor, aguarde enquanto trabalhamos para restaurar o serviço. Caso precise de assistência adicional, não hesite em entrar em contato com a nossa equipe de suporte técnico.',
			});

			const errorLogFolderPath: string = path.join(cwd(), '.log');
			const errorLogFilePath: string = path.join(
				errorLogFolderPath,
				'error.log',
			);
...
export default class AdminApiMiddlewares {
	public static async middlewareCheckToken(
		...
		try {
			...
			try {
				var [query] = await Sql.sqlMasterQuery(
		...
		return function (
			...
			upload(req, res, (err: any) => {
				if (err instanceof multer.MulterError || !req.files) {
					return MiddlewareUtil.handleMiddlewareError(res, {
						type: 400,
						message:
							'Por favor, forneça o número adequado de imagens para concluir a solicitação',
					});
				} else if (err) {
	...
	public static async middlewarePostPropaganda(
		...
		try {
			await MiddlewareUtil.validateDataForMiddlewarePostPropaganda(
				req.body,
				req.files,
			);
			...
			try {
				await Sql.sqlMasterQuery(
					'INSERT INTO propagandas (big, small) VALUES (?, ?);',
					[big.originalname, small.originalname],
				);
			} catch (err: any) {
				await S3.deleteFileFromS3Bucket(big.originalname);
	...
	public static async middlewarePostProduct(
		...
		try {
			await MiddlewareUtil.validateDataForMiddlewarePostProduct(
				req.body,
				req.files,
			);
			...
			await S3.uploadFileToS3Bucket(
				Object(files)[0].buffer.toString(),
				Object(files)[0].originalname,
				Object(files)[0].mimetype,
			);
	...
	public static async middlewarePutLogin(
		req: express.Request,
		res: express.Response,
		next: express.NextFunction,
	): Promise<void> {
		try {
			...
			await Sql.sqlMasterQuery(
				'UPDATE auth SET user = ?, password = ?, token = ? WHERE id = ?',
				[body.newUser, body.newPassword, body.newToken, 'only'],
			);
	...
	public static async middlewarePutText(
		...
		try {
			...
			const body: { id: string; table: string; column: string; value: string } =
				req.body;

			try {
				await Sql.sqlMasterQuery(
					'UPDATE ' + body.table + ' SET ' + body.column + ' = ? WHERE id = ?;',
					[body.value, body.id],
				);
			} catch (err: any) {
				if (
	...
	public static async middlewarePutImage(
		...
		try {
			await MiddlewareUtil.validateDataForMiddlewarePutImage(
				req.body,
				req.files,
			);
			...
			try {
				var [query] = await Sql.sqlMasterQuery(
					'SELECT ' + body.column + ' FROM ' + body.table + ' WHERE id = ?;',
					[body.id],
				);
			} catch (err: any) {
				if (
			...
			await S3.uploadFileToS3Bucket(
				Object(files)[0].buffer,
				Object(query)[0][body.column],
				Object(files)[0].mimetype,
			);
	...
	public static async middlewarePutPosition(
		...
		try {
			...
			for (let c = 0; c < body.ids.length; c++) {
				await Sql.sqlMasterQuery(
					'UPDATE ' + body.table + ' SET position = ? WHERE id = ?;',
					[String(c), Object(body).ids[c]],
				);
			}
		...
		try {
			...
			try {
				var [query]: [
					...
					'SELECT big, small FROM propagandas WHERE id = ?;',
					[body.id],
				);
			} catch (err: any) {
				if (
			...
			try {
				...
				await Sql.sqlMasterQuery('DELETE FROM propagandas WHERE id = ?;', [
					body.id,
				]);
	...
	public static async middlewareDeleteCategory(
		...
		try {
			...
			const [query2]: [
				...
				'SELECT id, image FROM products WHERE category = ?;',
				[Object(query1)[0].category],
			);
			...
			await Sql.sqlMasterQuery('DELETE FROM categories WHERE id = ?;', [
				body.id,
			]);

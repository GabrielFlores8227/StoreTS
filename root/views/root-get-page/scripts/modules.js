...
function buildHead(header) {
	window.document
		...
		.forEach((element) => element.setAttribute('content', header.color));
	window.document
		.querySelectorAll('link[rel="shortcut icon"]')
		.forEach((element) => element.setAttribute('href', header.icon));
	window.document
		.querySelectorAll('title')
		.forEach((element) => (element.innerText = header.title));
	window.document
		.querySelectorAll('meta[name="description"]')
		.forEach((element) => element.setAttribute('content', header.description));
...
function displayLoadingScreen() {
	const container = window.document.querySelector(
		'div[loading-screen-container] div',
	);
...
function buildProducts(products) {
	...
	keys.forEach((key, index) => {
		const template1 = productSectionContainer
			.querySelector('template')
			.cloneNode(true).content.children[0];
		...
		products[key].forEach((product) => {
			const productTemplate = productsContainer
				.querySelector('template')
				.cloneNode(true).content.children[0];
			productTemplate.setAttribute('product-' + product.id, '');

			const addToCartButton = productTemplate.querySelector(
			...
			addToCartButton.addEventListener('click', () => {
				...
				if (savedIds.includes(product.id)) {
					document
						.querySelector('aside[saved-aside-menu]')
						.classList.add('--on');
					return;
				}
			...
			if (product.off === 0) {
				...
				const productOldPrice = productTemplate.querySelector(
					'p[product-old-price]',
				);
				productOldPrice.innerText = convertToMoneyFormat(product.price);

				const productPrice = productTemplate.querySelector('p[product-price]');
				productPrice.innerText = convertToMoneyFormat(
					(Number(product.price) / 100) * (100 - Number(product.off)),
				);
			}

			const productInstallment = productTemplate.querySelector(
				'p[product-installment]',
			);
...
function buildFooter(footer) {
	...
	document.querySelector('p[footer-text]').innerText = footer.text;
	document
		.querySelector('a[whatsapp-link]')
		.setAttribute('href', 'https://wa.me/' + footer.whatsapp);
	document
		.querySelector('a[instagram-link]')
		.setAttribute(
			'href',
			'https://www.instagram.com/' + footer.instagram.replace('@', ''),
		);
	document
		.querySelector('a[facebook-link]')
		.setAttribute(
			'href',
			'https://www.facebook.com/' + footer.facebook.replace('@', ''),
		);
	document
		.querySelector('a[location-link]')
		.setAttribute('href', footer.location);
	document.querySelector('p[complete-store-info]').innerText =
	...
	images.forEach(function (image) {
		image.addEventListener('load', function () {
			setTimeout(() => {
				...
				window.document.querySelector('div[loading-progress-bar]').style.width =
					progress + '%';

				if (loadedCount === images.length) {
					setTimeout(() => {
						window.document
							.querySelector('div[loading-screen-container]')
							.classList.add('--off');
					}, 1750);
				}
			}, 1750);
...
function setImageSlider() {
	const imageSliderContainer = window.document.querySelector(
		'div[image-slider-container]',
	);
	const imageSlider = imageSliderContainer.querySelector('div');
	const numberOfPropagandas =
		imageSliderContainer.querySelectorAll('img[small-image]').length;
	let currentIndex = 0;
	let goingLeft = true;
	let isMoving = false;
...
function setProductSlider() {
	window.document
		.querySelectorAll('div[product-slider-container]')
	...
	window.addEventListener('resize', () => {
		window.document
			...
			.forEach((element) => {
				if (
					!element.classList.contains('--special') &&
					element.scrollWidth - element.clientWidth < 200
				) {
					element.classList.add('--special');
